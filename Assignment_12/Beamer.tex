...
\end{align}
\textbf{Case 4: }Let $a,b,c,d\not=0$. Considering the following case,
\begin{align}
\vec{A}\vec{X} = \vec{u} \not= 0\\
\implies\myvec{a&b\\c&d}\myvec{x_1\\x_2} &=\myvec{u_1\\u_2}\not= \vec{0}\label{eqComp}\\
...
du_1-bu_2 & = 0\label{eqU1}\\
au_2-cu_1 & = 0\label{eqU2}\\
\intertext{Solving \eqref{eqU1} and \eqref{eqU2},}
u_1 & = 0\\
u_2 & = 0\\
\intertext{Hence we get,}
\vec{u} = \myvec{u_1\\u_2} &= \myvec{0\\0}\label{eqI4}
\end{align}
Thus $\vec{A}\vec{X}=\vec{u}$ holds true if and only if $\vec{u}=0$.\\
In \eqref{eqI1}, \eqref{eqI2}, \eqref{eqI3} and \eqref{eqI4}, we can see that $\vec{A}\vec{X}=0$ has only one trivial solution i.e $x_1=x_2=0$ in all cases. Hence proved, the equation $\vec{AX}$=0 has only one trivial solution $x_1=x_2=0$
